...
# Stablecoins y fiat-pegged: se mueven por diseño ~0%, contaminan el top smallcap
STABLE_BASES = ("USDC", "FDUSD", "TUSD", "DAI", "USDP", "AEUR", "EUR", "GBP", "BRL", "ARS", "BIDR", "IDRT", "NGN", "TRY", "UAH", "VAI", "XUSD")

def is_stable_or_fiat(symbol: str) -> bool:
    base = symbol[:-4] if symbol.endswith("USDT") else symbol
    return base in STABLE_BASES or base.endswith("UP") or base.endswith("DOWN") or base.endswith("BULL") or base.endswith("BEAR")

def get_all_usdt_tickers(min_volume: float = 100_000, max_volume: float = 30_000_000) -> list:
    """Universo smallcap: banda de volumen 24h. Piso para evitar monedas muertas,
    techo para excluir gigantes (BTC/ETH/SOL...) que necesitan mucho capital para moverse."""
    ...
    return [
        ...
        and min_volume < float(d["quoteVolume"]) < max_volume
        and not is_stable_or_fiat(d["symbol"])
    ]
...
def get_intraday_momentum(symbol: str) -> dict | None:
    ...
    Este es el filtro principal: detecta lo que se mueve AHORA, no ayer."""
    try:
    ...
    # Si el precio está en el pico, la entrada es high_4h con ese descuento.
